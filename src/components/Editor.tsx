...
export default function Editor({
	...
	const [contextChapterIds, setContextChapterIds] = useState<Set<string>>(
		new Set(),
	);
	...
	const nonSpaceLength = useCallback(
		(text: string) => (text || "").replace(/\s+/g, "").length,
		[],
	);

	const currentCount = useMemo(
		() => nonSpaceLength(content),
		[content, nonSpaceLength],
	);
	...
	}, [otherChapters, contextChapterIds, nonSpaceLength]);
	...
	const handlePlotGenerate = async () => {
		...
		try {
			...
			const errorMessage = e instanceof Error ? e.message : "请稍后重试";
			toast({
				title: "生成失败",
	...
	function buildDiffHtml(original: string, corrected: string): string {
		...
		function highlightLineDiff(o: string, c: string): string {
			...
			for (let i = 0; i < m; i++) {
				for (let j = 0; j < n; j++) {
					if (normalizeChar(a[i]) === normalizeChar(b[j]))
						dp[i + 1][j + 1] = dp[i][j] + 1;
			...
			while (i > 0 || j > 0) {
				if (
					...
					out.push({ t: "same", ch: b[j - 1] });
					i--;
					j--;
				} else if (j > 0 && (i === 0 || dp[i][j - 1] >= dp[i - 1][j])) {
					out.push({ t: "add", ch: b[j - 1] });
					j--;
				} else if (i > 0) {
					// 删除 a[i-1]
					out.push({ t: "del", ch: a[i - 1] });
					i--;
				}
			}
			out.reverse();
		...
		for (let i = 0; i < maxLines; i++) {
			...
			if (i >= oLines.length) {
				// 新增整行
				resultLines.push(
					`<span class="bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 font-medium">${escapeHtml(c)}</span>`,
				);
				continue;
			}
			if (i >= cLines.length) {
				// 删除整行
				resultLines.push(
					`<span class="line-through text-red-600 dark:text-red-400 opacity-80">${escapeHtml(o)}</span>`,
				);
				continue;
			}
...
4. 如果某段没有错误，原样输出`;
			...
			toast({
				title: "校对完成",
				description: "已完成智能校对，如不满意可使用 Ctrl+Z 撤销",
				duration: 4000,
			});
		} catch (e: unknown) {
	...
	const handleGenerate = async () => {
		...
		try {
			...
			const otherChaptersContext =
				compressedContext && useCompressedContext
					? `【压缩剧情要点】\n${compressedContext}`
					: rawContext;
...
请根据用户的指令和以上上下文信息进行创作。`;
	...
	const budgetLabel =
		budgetValue === -1 ? "动态" : budgetValue === 0 ? "关闭" : budgetValue;
	...
	const handleCompress = async () => {
		...
		try {
			...
			const modelId = compressModel || selectedModel || getCurrentModel();
	...
	return (
		<div className="flex flex-col h-full bg-background relative">
			<div className="px-3 py-2 sm:p-4 border-b flex justify-between items-center">
				<h2 className="text-lg sm:text-2xl font-bold font-headline">
					{chapter.title}
				</h2>
				<div className="flex items-center gap-1 sm:gap-2">
					<div className="text-[11px] sm:text-xs text-muted-foreground whitespace-nowrap flex-shrink-0">
						字数：<span className="font-medium">{currentCount}</span>
					</div>
			...
			{showFab && (
				<div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-20 flex items-center gap-2 bg-background/80 backdrop-blur-md border rounded-full px-2 py-1 shadow-md">
					<Button
						...
						variant="ghost"
						className="h-8 px-3"
						onClick={() => setIsPlotDialogOpen(true)}
					>
			...
			<Dialog
				...
				onOpenChange={(open) => {
					setIsPlotDialogOpen(open);
					if (!open) {
						setPlotResult("");
						setPlotDissatisfaction("");
					}
				}}
			>
							...
							{!plotResult ? (
								<>
									<div className="space-y-2">
										<div className="flex items-center justify-between">
											...
											<div className="flex gap-2">
												<Select
													onValueChange={handleCommunityPromptSelectForPlot}
														...
														<SelectValue
															placeholder={
																<div className="flex items-center gap-1">
																	...
																	<span className="text-xs">社区提示词</span>
																</div>
															}
														/>
													</SelectTrigger>
									...
									<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
										...
										<div className="space-y-2">
											<Label>最大输出</Label>
											<Select
												value={String(maxTokens[0])}
												onValueChange={(v) => setMaxTokens([parseInt(v, 10)])}
											>
												<SelectTrigger>
													<SelectValue placeholder="选择最大输出" />
												</SelectTrigger>
												<SelectContent>
													{[2048, 3072, 4096, 6144, 8192].map((n) => (
								...
								<>
									...
									<div className="space-y-2">
										...
										<Textarea
											rows={3}
											value={plotDissatisfaction}
											onChange={(e) => setPlotDissatisfaction(e.target.value)}
											placeholder="例如：冲突弱、推进慢、缺少反转、主角动机单薄……"
										/>
										<p className="text-xs text-muted-foreground">
											填写后点击"重新抽卡"将根据不满意点重新生成
										</p>
									</div>
								</>
							)}
				...
				<DialogContent className="sm:max-w-[720px]">
					...
					<ScrollArea className="max-h-[65vh] pr-4">
						<div className="rounded-md border p-4 bg-muted/30">
							<div className="text-sm whitespace-pre-wrap">{proofDiffHtml}</div>
						</div>
					</ScrollArea>
					<DialogFooter>
						<DialogClose asChild>
			...
			>
				<DialogContent className="sm:max-w-[500px]">
					...
					<div className="space-y-4 py-4">
						<div className="space-y-2">
							...
							>
								<SelectTrigger>
									<SelectValue
										placeholder={isModelListLoading ? "加载中..." : "选择模型"}
									/>
								</SelectTrigger>
								<SelectContent>
									{availableModels.map((m) => (
					...
					<DialogFooter>
						<DialogClose asChild>
							<Button variant="secondary" disabled={isProofing}>
								取消
							</Button>
						</DialogClose>
							...
							<Accordion type="single" collapsible className="w-full">
													...
													otherChapters.map((chap) => (
														...
														>
															<Checkbox
																...
																onCheckedChange={() =>
																	handleContextChapterToggle(chap.id)
																}
															/>
															<label
										...
										<div className="mt-3 space-y-2">
											...
											{reachCompressThreshold && (
												<div className="rounded-md border p-3 space-y-2">
													...
													<div className="text-xs text-muted-foreground">
														选择章节达到10万字时，可以先压缩为剧情要点，生成时优先使用。
													</div>
													<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
														<div className="space-y-2">
															<Label>压缩使用模型</Label>
															<Select
																value={compressModel || selectedModel}
																onValueChange={setCompressModel}
																disabled={isModelListLoading}
															>
																<SelectTrigger>
																	<SelectValue
														...
														<div className="space-y-2">
															<Label>压缩最大输出</Label>
															<Select
																value={String(compressMaxTokens)}
																onValueChange={(v) => {
								...
								<AccordionItem value="item-1">
									...
									<AccordionContent className="pt-4 space-y-6">
										...
										<div className="grid gap-2">
											<Label>
												温度 (Temperature): {temperature[0].toFixed(2)}
											</Label>
											<Slider
												value={temperature}
