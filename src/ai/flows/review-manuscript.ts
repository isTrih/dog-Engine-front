...
const ReviewManuscriptInputSchema = z.object({
	manuscript: z
		...
		.max(10000)
		.describe(
			"The manuscript content to be reviewed, with a maximum of 10,000 characters.",
		),
	modelId: z
		.string()
		.optional()
		.describe("Optional model ID to use when reviewing."),
});
export type ReviewManuscriptInput = z.infer<typeof ReviewManuscriptInputSchema>;

const ReviewDecisionSchema = z
	.enum(["过稿", "拒稿"])
	.describe('The review decision, either "过稿" (Pass) or "拒稿" (Reject).');
...
const ReviewManuscriptOutputSchema = z.object({
	...
	reason: z
		.string()
		.describe(
			"The detailed reason for the decision. If it passes, list the strengths. If it is rejected, list the weaknesses.",
		),
});
export type ReviewManuscriptOutput = z.infer<
	typeof ReviewManuscriptOutputSchema
>;

export async function reviewManuscript(
	input: ReviewManuscriptInput,
): Promise<ReviewManuscriptOutput> {
