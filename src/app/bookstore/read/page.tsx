...
function ChapterReader() {
	...
	const fetchChapter = async (fetchUrl: string) => {
		...
		router.replace(
			`/bookstore/read?url=${encodeURIComponent(fetchUrl)}&sourceId=${sourceId}`,
			{ scroll: false },
		);
		try {
			const res = await fetch(
				`/api/bookstore/chapter?url=${encodeURIComponent(fetchUrl)}&sourceId=${sourceId}`,
			);
			if (!res.ok) {
		...
		<div className="flex flex-col min-h-screen">
			<Header>
				<div className="flex-grow flex justify-center items-center">
					<h1
						className="text-lg font-semibold truncate font-headline"
						title={chapter.title}
					>
						{chapter.title}
					</h1>
			...
			<main className="flex-grow container mx-auto px-4 md:px-6 lg:px-8 py-8">
				...
				<div className="flex justify-between items-center mt-8 max-w-3xl mx-auto">
					<Button
						onClick={() =>
							chapter.prevChapterUrl && fetchChapter(chapter.prevChapterUrl)
						}
					...
					<Button
						onClick={() =>
							chapter.nextChapterUrl && fetchChapter(chapter.nextChapterUrl)
						}
