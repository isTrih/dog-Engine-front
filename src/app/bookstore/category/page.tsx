		...
		async function fetchBooks() {
			...
			try {
				const res = await fetch(
					`/api/bookstore/category?url=${encodeURIComponent(url)}&sourceId=${sourceId}`,
				);
				if (!res.ok) {
			...
			<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
				{books.map((book, index) => (
					...
					>
						...
						<Link
							href={`/bookstore/book?url=${encodeURIComponent(book.detailUrl)}&sourceId=${book.sourceId}`}
							passHref
						>
							<div className="relative aspect-[3/4] w-full">
								<Image
									src={
										book.cover
											? `/api/proxy-image?url=${encodeURIComponent(book.cover)}`
							...
							<div className="p-3">
								<h3 className="text-sm font-bold font-headline line-clamp-1">
									{book.title}
								</h3>
